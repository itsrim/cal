...
export default function HistoryTab() {
  ...
  const sameDay = (a: number, b: number) => startOfDay(new Date(a)) === startOfDay(new Date(b));
  ...
  const totalQtyForDay = useMemo(() => {
    return itemsForSelectedDay.reduce((sum, it) => sum + (it.quantity ?? 100), 0);
  }, [itemsForSelectedDay]);

  const totalKcalForDay = useMemo(() => {
    return itemsForSelectedDay.reduce((sum, it) => {
      const base = (it.nutriments as any)?.['energy-kcal_100g'] ?? (it.nutriments as any)?.energy_kcal_100g ?? null;
  ...
  return (
    <>
      <CalendarStrip>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          {daysStrip.map(d => {
            ...
            return (
              <DayPill key={d.toISOString()} selected={selected} onPress={() => setSelectedDate(d)}>
      ...
      <Row>
        ...
        <Value>{totalQtyForDay} g · {Math.round(totalKcalForDay)} kcal</Value>
      </Row>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        {itemsForSelectedDay.length === 0 ? (
          ...
          const k = (nf as any)?.['energy-kcal_100g'] ?? (nf as any)?.energy_kcal_100g ?? null;
          return (
            <Card key={it.id}>
              <Row>
                <LeftRow>
                  <IconButton onPress={() => removeItem(it.id)} accessibilityLabel="Supprimer définitivement">
                    <Trash>🗑️</Trash>
                  </IconButton>
                  <ProductName>
                    {it.product_name}
                ...
                <Row>
                  <QtyInput
                    ...
                    onChangeText={async (text) => {
                      ...
                      const next = saved.map(s => s.id === it.id ? { ...s, quantity: v } : s);
                      setSaved(next);
                      await AsyncStorage.setItem(storageKey, JSON.stringify(next));
                    }}
                  />
              ...
              <Row>
                ...
                <Value>{k !== null ? `${Math.round((k * (it.quantity ?? 100)) / 100)} kcal` : '—'}</Value>
              </Row>
              <Row>
                <Label>Lipides</Label>
